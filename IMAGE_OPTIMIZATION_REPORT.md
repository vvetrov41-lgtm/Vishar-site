...
## Phase 1A Markup/JS Serving Update

Files changed to serve optimized formats with fallback:
- `index.html` (portfolio grid now renders `<picture>` with AVIF/WebP sources + JPG fallback; lightbox prefers optimized source when available)
- `cover-up-tattoo-manchester/index.html` (before/after gallery cards now render `<picture>` with AVIF/WebP + original fallback)
- `black-and-grey-realism-manchester/index.html` (gallery card renderer now outputs `<picture>` with AVIF/WebP + original fallback)
- `colour-realism-tattoo-manchester/index.html` (gallery card renderer now outputs `<picture>` with AVIF/WebP + original fallback)

Remaining risks:
- This branch currently shows no committed `.webp`/`.avif` files under `assets/`; if optimized derivatives are missing at deploy time, browsers will use original fallback images.
- Full performance gain depends on ensuring generated AVIF/WebP sidecars are present in the deployed branch.
