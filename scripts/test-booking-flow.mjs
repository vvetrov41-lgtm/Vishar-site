...
function validEnquiry(overrides = {}) {
  return {
    ...
    images: [{ name: 'reference.jpg', type: 'image/jpeg', data: JPEG_REFERENCE }],
    source: '/booking/',
    ...overrides,
  };
}
...

{
  ...
  const { response, payload } = await send(
    validEnquiry({
      images: [
        { name: 'reference-1.jpg', type: 'image/jpeg', data: JPEG_REFERENCE },
        { name: 'reference-2.jpg', type: 'image/jpeg', data: JPEG_REFERENCE },
      ],
    }),
    async (url) => {
...
{
  const { response, payload } = await send(
    validEnquiry({ images: [{ name: 'reference.exe', type: 'image/jpeg', data: btoa('not an image') }] }),
    async () => {
      throw new Error('Telegram must not be called for a fake image.');
    }
  );
  assert.equal(response.status, 400);
  assert.match(payload.error, /wrong file type/);
}

{
  let uploadedFilename = '';
  const { response } = await send(
    validEnquiry({ images: [{ name: 'reference.exe', type: 'image/jpeg', data: JPEG_REFERENCE }] }),
    async (url, init) => {
      if (String(url).endsWith('/sendMessage')) return new Response('{}', { status: 200 });
      if (String(url).endsWith('/sendDocument')) {
        uploadedFilename = init.body.get('document').name;
  ...
  assert.equal(response.status, 200);
  assert.equal(uploadedFilename, 'reference-1.jpg');
}
...
console.log('Booking flow tests passed: contact validation, verified reference images and delivery reporting.');
