...
for (const name of ['core', 'operations', 'communications']) {
  assert(ids[name].length < 30, `${name} reaches/exceeds Builder limit: ${ids[name].length}`);
  assert(ids[name].length <= 25, `${name} must retain at least five operation slots of headroom`);
}
...
for (const id of gmailIds) {
  ...
  assert(!ids.operations.includes(id), `Operations must not contain ${id}`);
}

const splitUnion = new Set([...ids.core, ...ids.operations, ...ids.communications]);
const monolithSet = new Set(ids.monolith);
assert.equal(splitUnion.size, monolithSet.size, 'three-way split union must match canonical operation count');
for (const id of monolithSet) assert(splitUnion.has(id), `split union missing canonical operation ${id}`);
