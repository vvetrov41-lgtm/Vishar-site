...
import monzoGateway, { __testing as monzoGatewayTesting } from '../workers/monzo-api-gateway.js';
...
const PAYMENT_ORIGIN = 'https://pay.vishartattoo.com';
...
await test('GET resolves one opaque id through the single backend RPC and redirects to Monzo', async () => {
  ...
  await withGlobalFetch(async (url, options) => {
    ...
    const response = await paymentWorker.fetch(new Request(`${PAYMENT_ORIGIN}${PAYMENT_PATH}`), env);
...
await test('existing Monzo gateway intercepts the dedicated payment hostname before owner management handling', async () => {
  let backendCalls = 0;
  await withGlobalFetch(async () => {
    ...
    const response = await monzoGateway.fetch(
      new Request(`${PAYMENT_ORIGIN}${PAYMENT_PATH}`),
      env,
    );
    assert.equal(response.status, 302);
    assert.equal(response.headers.get('location'), MONZO_URL);
  ...
  assert.equal(monzoGatewayTesting.PAYMENT_HOST, 'pay.vishartattoo.com');
});

await test('the whole dedicated payment hostname fails closed outside one exact payment path', async () => {
  ...
  await withGlobalFetch(async () => {
    ...
    for (const path of [
      ...
      const response = await monzoGateway.fetch(
        new Request(`${PAYMENT_ORIGIN}${path}`),
        localEnv,
      );
...
await test('the apex hostname is not the payment hostname', async () => {
  assert.notEqual(monzoGatewayTesting.PAYMENT_HOST, 'vishartattoo.com');
});
...
await test('non-GET methods fail before rate limiting or backend access', async () => {
  ...
  await withGlobalFetch(async () => {
    ...
    const response = await paymentWorker.fetch(new Request(`${PAYMENT_ORIGIN}${PAYMENT_PATH}`, { method: 'POST' }), localEnv);
...
await test('invalid path and query strings fail before rate limiting or backend access', async () => {
  ...
  await withGlobalFetch(async () => {
    ...
    for (const url of [
      `${PAYMENT_ORIGIN}/pay-by-bank-transfer/not-a-uuid`,
      `${PAYMENT_ORIGIN}${PAYMENT_PATH}?next=https://evil.example`,
      `${PAYMENT_ORIGIN}${PAYMENT_PATH}/extra`,
    ]) {
...
await test('production Monzo config owns a dedicated payment Custom Domain without introducing a new backend credential', async () => {
  ...
  assert.match(config, /pattern = "pay\.vishartattoo\.com"[^\n]*custom_domain = true/);
