...
const FORBIDDEN_INPUTS = [
  'sql', 'query', 'rpc', 'table', 'oauth_client_id', 'integration_key',
  'access_token', 'refresh_token', 'client_secret', 'service_role',
];
...

{
  ...
  assert.equal((await json(response)).error, 'oauth_token_required');
...

{
  ...
  assert.deepEqual(payload.error.data.supported, [MCP_PROTOCOL_VERSION]);
}

{
  const response = await handleMcpRequest(request('tools/list', {}, { methodHeader: 'server/discover' }), env());
  assert.equal(response.status, 400);
  assert.equal((await json(response)).error.code, -32020);
}

{
  ...
  assert.deepEqual(payload.result.supportedVersions, [MCP_PROTOCOL_VERSION]);
...

{
  ...
  for (const tool of payload.result.tools) {
    ...
    for (const forbidden of FORBIDDEN_INPUTS) {
      assert(!properties.includes(forbidden), `${tool.name} must not expose ${forbidden} as model input`);
    }
  }
}

{
  const service = { async fetch() { throw new Error('not called during tools/list'); } };
  ...
  assert(!names.some((name) => /^crm_email_.*send/i.test(name)), 'Gmail MCP projection remains draft-only');
  for (const tool of payload.result.tools) {
    const properties = Object.keys(tool.inputSchema?.properties || {});
    for (const forbidden of FORBIDDEN_INPUTS) assert(!properties.includes(forbidden));
  }
}

{
  ...
  assert.equal(payload.result.structuredContent.items[0].id, ARTIST);
  assert.deepEqual(payload.result.structuredContent.items[0].capabilities, ['view_enquiries', 'view_projects']);
  assert.equal(calls.length, 2);
  assert(calls.every((call) => !String(call.init.headers.apikey).startsWith('sb_secret_')));
}

for (const forbidden of ['sql', 'rpc', 'table', 'integration_key', 'access_token']) {
  let called = false;
  const response = await handleMcpRequest(
    request('tools/call', {
      name: 'crm_list_enquiries',
      arguments: { artist_id: ARTIST, [forbidden]: 'synthetic' },
    }),
    env(),
  ...
  assert.equal(payload.result.structuredContent.error, 'forbidden_argument');
  assert.equal(called, false, `${forbidden} is rejected before any Data API call`);
}
...

{
  ...
  const fetchImpl = async (url, init = {}) => {
    ...
    throw new Error('row read must not happen without capability');
  };
  const response = await handleMcpRequest(
  ...
  const payload = await json(response);
  assert.equal(payload.result.isError, true);
  assert.equal(payload.result.structuredContent.error, 'permission_denied');
...

{
  ...
  const payload = await json(response);
  assert.equal(payload.result.isError, false);
  assert.equal(payload.result.structuredContent.untrusted_content, true);
...
{
  const service = {
    async fetch() { return Response.json({ error: 'provider_secret_internal_detail' }, { status: 403 }); },
  };
  const response = await handleMcpRequest(
    request('tools/call', {
  ...
  assert.equal(payload.result.isError, true);
  assert.equal(payload.result.structuredContent.error, 'permission_denied');
  assert.doesNotMatch(JSON.stringify(payload), /provider_secret_internal_detail/);
}
