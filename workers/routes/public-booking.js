...
<label id="discoveryDetailWrap" class="span conditional"><span id="discoveryDetailLabel"></span><input id="discoverySourceDetail" name="discoverySourceDetail" maxlength="200"></label>
...
<script>(function(){'use strict';var form=document.getElementById('booking'),submit=document.getElementById('submit'),status=document.getElementById('status'),files=document.getElementById('references'),preferred=document.getElementById('preferredReply'),phone=document.getElementById('phone'),instagram=document.getElementById('instagram'),discovery=document.getElementById('discoverySource'),discoveryWrap=document.getElementById('discoveryDetailWrap'),discoveryLabel=document.getElementById('discoveryDetailLabel'),discoveryDetail=document.getElementById('discoverySourceDetail');var keyName='vishar.public.enquiry.${slug}';function key(){var value='';try{value=sessionStorage.getItem(keyName)||'';}catch(e){}if(value)return value;if(!crypto||!crypto.randomUUID)throw new Error('Please update your browser and try again.');value=crypto.randomUUID();try{sessionStorage.setItem(keyName,value);}catch(e){}return value}function clearKey(){try{sessionStorage.removeItem(keyName)}catch(e){}}function sync(){phone.required=preferred.value==='WhatsApp';instagram.required=preferred.value==='Instagram'}function syncDiscovery(){var value=discovery.value,visible=value==='other'||value==='referral'||value==='ai';discoveryWrap.style.display=visible?'block':'none';discoveryDetail.required=value==='other';if(value==='other')discoveryLabel.textContent='Please tell us where you found ${artist} *';else if(value==='referral')discoveryLabel.textContent='Who recommended ${artist}? (optional)';else if(value==='ai')discoveryLabel.textContent='Which AI service? (optional)';else{discoveryLabel.textContent='';discoveryDetail.value=''}}preferred.addEventListener('change',sync);discovery.addEventListener('change',syncDiscovery);sync();syncDiscovery();form.addEventListener('submit',async function(event){event.preventDefault();sync();syncDiscovery();if(!form.reportValidity())return;var selected=Array.from(files.files||[]);if(selected.length<1||selected.length>3||selected.some(function(file){return file.size>4*1024*1024})){status.textContent='Please attach 1-3 reference images, up to 4 MB each.';return}submit.disabled=true;submit.textContent='Sending...';status.textContent='Uploading your enquiry securely...';try{var raw=new FormData(form),payload=new FormData();['name','email','phone','instagram','preferredReply','travellingFrom','discoverySource','discoverySourceDetail','projectType','placement','size','coverUp','timing','idea','website'].forEach(function(name){payload.append(name,raw.get(name)||'')});var params=new URLSearchParams(location.search);payload.append('idempotencyKey',key());payload.append('privacyAcknowledged',raw.get('privacyAcknowledged')?'true':'false');payload.append('privacyNoticeVersion','${privacyVersion}');payload.append('source',location.pathname.slice(0,200));payload.append('landingPage',location.href.slice(0,500));payload.append('referrer',String(document.referrer||'').slice(0,500));payload.append('utmSource',(params.get('utm_source')||'').slice(0,120));payload.append('utmMedium',(params.get('utm_medium')||'').slice(0,120));payload.append('utmCampaign',(params.get('utm_campaign')||'').slice(0,160));payload.append('utmContent',(params.get('utm_content')||'').slice(0,160));payload.append('utmTerm',(params.get('utm_term')||'').slice(0,160));selected.forEach(function(file){payload.append('references',file,file.name)});var response=await fetch('${formPath}',{method:'POST',body:payload,credentials:'same-origin'});var result=await response.json().catch(function(){return {}});if(!response.ok||!result.ok){if(response.status>=400&&response.status<500)clearKey();throw new Error(result.error||'The enquiry could not be sent.')}clearKey();form.reset();sync();syncDiscovery();status.textContent=result.reference?'Thank you. Your enquiry is saved as '+result.reference+'.':'Thank you. Your enquiry has been saved.';submit.textContent='Enquiry sent'}catch(error){status.textContent=error&&error.message?error.message:'The enquiry could not be sent. Please try again.';submit.disabled=false;submit.textContent='Try again'}})})();</script></body></html>`;
}
