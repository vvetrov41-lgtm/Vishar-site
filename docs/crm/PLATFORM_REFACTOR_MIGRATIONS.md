...
The first unfinished phase that actually begins after the current `0082` lineage may claim `0083`. Later phases claim the next free number from the then-current repository state, not from the stale planning table.
