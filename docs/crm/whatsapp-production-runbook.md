...
Do not trust the values below as a future handoff. They describe the validated
RC24 snapshot and must be checked again before acting.

| Check | RC24 expected state |
|---|---|
| Release branch | `release/private-crm-rc24-whatsapp` |
| Exact validated head | `892df2ab512a940d88a9bc3a287551f7611fbc6f` |
| Production DB | through migration `0048` only |
| Pending WhatsApp DB migrations | `0049`, `0050`, `0051` |
| Production WhatsApp integrations | `0` |
| Production WhatsApp outbox jobs | `0` |
| Retained staging DB | migration `0044`, untouched |
| Scheduled drain Worker | not activated by RC24 validation |
| Public webhook Custom Domain | must be independently verified before deploy |
| Real Meta messages | none sent by RC24 validation |
...
For the RC24 snapshot the dry-run must propose exactly the still-pending
WhatsApp migrations in repository order. If another migration appears, if the
...
- migration history contains `0049`, `0050`, `0051` exactly once and in order;
