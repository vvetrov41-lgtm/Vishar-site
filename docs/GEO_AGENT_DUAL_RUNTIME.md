...
Runtime-local files include:

- `.geo-agent-local/<runtime>/runtime/runtime-profile.json`
- `.geo-agent-local/<runtime>/runtime/birth-plan.json`
- `.geo-agent-local/<runtime>/STATE.md` after a command that updates operational state
...
The tracked `geo_agent/STATE.md` remains only for legacy direct-CLI compatibility. Launcher-driven Codex sessions must read `.geo-agent-local/codex/STATE.md` when it exists; launcher-driven Claude Code sessions must read `.geo-agent-local/claude/STATE.md` when it exists.
...
Do not rerun first-time intake for this Vishar project. For normal topic work, read `AGENTS.md`, `.geo-topic-agent-runtime/AGENTS.md`, `geo_agent/PROJECT_PROFILE.md`, `geo_agent/config/geo_agent_config.json`, and the matching runtime-local STATE file when present before GEO/AEO work. Run setup only when adapting a new checkout or verifying runtime detection.
...
Both profiles use the same canonical runtime and copied shared project facts, while setup output remains isolated under separate ignored runtime-local roots. The tracked shared project config must remain byte-for-byte unchanged after either setup command. Runtime-local `STATE.md` files are created later by commands that call the runtime's state updater.
