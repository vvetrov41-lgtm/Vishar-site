...
const PRODUCTION_ONBOARDING_ARTIST_ID = 'a1111111-1111-4111-8111-111111111111';
...
function assertProvisioningResponse(value: unknown): WhatsAppProvisioningResult {
  if (!value || typeof value !== 'object') throw new ApiError('WhatsApp provisioning returned an invalid response.');
  const row = value as Record<string, unknown>;
  if (
    row.ok !== true
    || row.integration_key !== 'vladimir-production'
...
export function createWhatsAppConnectionsApi(client: CrmClient) {
  ...
  return {
    ...
    async provisionProductionWhatsApp(
      ...
      if (artist.id !== PRODUCTION_ONBOARDING_ARTIST_ID || artist.slug !== 'vladimir') {
        throw new ApiError('Production WhatsApp onboarding is currently scoped only to Vladimir.');
      }
      if (signup.event !== 'FINISH') throw new ApiError('Meta Embedded Signup did not finish.');

      const session = await client.auth.getSession();
      const accessToken = session.data?.session?.access_token;
      if (!accessToken) throw new ApiError('Your CRM session expired. Sign in again before connecting WhatsApp.');
      ...
      return assertProvisioningResponse(payload);
    },
  };
}
