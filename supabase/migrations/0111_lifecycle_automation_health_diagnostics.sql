-- 0111_lifecycle_automation_health_diagnostics.sql
--
-- Extend the existing read-only Lifecycle Automation health projection with
-- bounded runtime timing diagnostics. These fields help an operator distinguish
-- a quiet system from a growing pending backlog without exposing client data,
-- message copy, destinations, provider details or raw failures.
--
-- The input signature and execute ACL stay unchanged. Existing CRM clients can
-- ignore the appended columns, so this is backward compatible at the RPC call
-- boundary while the later UI slice can adopt the diagnostics explicitly.

-- PostgreSQL cannot change OUT columns with CREATE OR REPLACE, so recreate the
-- function atomically inside this migration while preserving the exact input
...
comment on function public.get_lifecycle_automation_health(uuid) is
  'Returns bounded Artist lifecycle health plus pending/completed/failed runtime timing diagnostics without client data, message copy, destinations or raw provider errors.';
