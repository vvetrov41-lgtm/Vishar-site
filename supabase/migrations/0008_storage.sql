...
set search_path = pg_catalog, public, crm_private
as $$
  select public.can_manage_crm() and public.crm_storage_object_is_known(p_name);
$$;

-- Write access: the same identity checks, plus the requirement that a pending
-- manifest already exists. Objects are never uploaded ahead of their database
-- row, so a partial upload always leaves something reconcilable.
...
revoke all on function public.crm_storage_object_is_known(text) from public;
revoke all on function public.crm_storage_object_readable(text) from public;
revoke all on function public.crm_storage_object_writable(text) from public;
...
create policy crm_files_bucket_visible on storage.buckets
  ...
  using (id = 'crm-files' and public.is_active_user());
