...
--
-- Harden the future Monzo reconciliation adapter before it is connected:
-- provider event ids are artist/account scoped immutably, and UUID candidate
-- selection avoids relying on an aggregate that PostgreSQL 15 does not define.
-- No live provider call or automatic payment settlement is enabled here.
