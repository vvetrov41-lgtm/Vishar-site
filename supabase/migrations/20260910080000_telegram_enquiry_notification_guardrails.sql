-- 20260910080000_telegram_enquiry_notification_guardrails.sql
--
-- New-enquiry Telegram delivery must follow effective CRM access and the
-- profile's explicit Telegram preference. Membership labels such as owner,
-- artist or manager are not delivery capabilities and must not be used as a
-- routing gate. Keeping the predicate in one helper also prevents recovery and
-- normal routing from drifting apart again.
...
revoke all on function crm_private.telegram_notification_recipient_eligible(uuid,uuid,uuid)
  from public, anon, authenticated, service_role;

comment on function crm_private.telegram_notification_recipient_eligible(uuid,uuid,uuid) is
  'Single fail-closed predicate for profile-scoped Telegram notification delivery. Uses effective artist/workspace access, active destination and explicit channel preference; intentionally does not gate on membership role labels.';
...
comment on function public.service_route_telegram_enquiry_notification(uuid,text) is
  'Turns one leased enquiry Telegram outbox job into deduplicated profile notifications using the shared effective-access recipient predicate. It performs no provider call and returns no chat id.';
...
comment on function public.service_recover_telegram_enquiry_outbox(uuid) is
  'Requeues only a destination-unavailable dead enquiry job with no delivery evidence and a currently eligible Telegram recipient. Uses the same recipient predicate as normal routing.';
