...
select ok(
  position("v_status = 'new'" in lower(pg_get_functiondef(
...
select ok(
  position("'declined'" in lower(pg_get_functiondef(
    'crm_private.client_request_information_is_actionable(uuid,uuid)'::regprocedure))) > 0
  and position("'converted'" in lower(pg_get_functiondef(
    'crm_private.client_request_information_is_actionable(uuid,uuid)'::regprocedure))) > 0
  and position("'closed'" in lower(pg_get_functiondef(
    'crm_private.client_request_information_is_actionable(uuid,uuid)'::regprocedure))) > 0,
  'terminal enquiry states cannot reopen intake questions');
select ok(
  position("t.source <> 'enquiry'" in lower(pg_get_functiondef(
    'crm_private.client_request_information_is_actionable(uuid,uuid)'::regprocedure))) > 0
...
select ok(
  position("new.status := 'superseded'" in lower(pg_get_functiondef(
...
select ok(
  position("a.action_type = 'request_information'" in lower(pg_get_functiondef(
    'crm_private.enqueue_enquiry_client_ai()'::regprocedure))) > 0
  and position("set status = 'superseded'" in lower(pg_get_functiondef(
    'crm_private.enqueue_enquiry_client_ai()'::regprocedure))) > 0,
...
select ok(
  position("new.link_state := 'linked'" in lower(pg_get_functiondef(
