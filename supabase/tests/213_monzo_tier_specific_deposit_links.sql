-- 213_monzo_tier_specific_deposit_links.sql
-- Synthetic-only validation that reusable Monzo destinations are selected from
-- the immutable server-calculated payment request amount.
...
select lives_ok(
  $$select public.configure_monzo_easy_bank_transfer_tier_urls(
      'a1111111-1111-4111-8111-111111111111',
  ...
  'finance-authorised owner configures four distinct tier destinations'
);

select throws_ok(
  $$select public.configure_monzo_easy_bank_transfer_tier_urls(
      'a1111111-1111-4111-8111-111111111111',
      'https://monzo.com/pay/r/same-link',
      'https://monzo.com/pay/r/same-link',
      'https://monzo.com/pay/r/synthetic-other-150',
      'https://monzo.com/pay/r/synthetic-other-250',
      true
    )$$,
  '22023', 'each deposit tier must use a distinct Monzo Easy Bank Transfer URL',
  'configuration rejects one reusable destination being reused across amounts'
);

select is(
  public.get_monzo_easy_bank_transfer_tier_urls('a1111111-1111-4111-8111-111111111111') ->> '50',
  'https://monzo.com/pay/r/synthetic-tier-50',
  'safe settings expose the GBP 50 destination'
);
select is(
  public.get_monzo_easy_bank_transfer_tier_urls('a1111111-1111-4111-8111-111111111111') ->> '250',
  'https://monzo.com/pay/r/synthetic-tier-250',
  'safe settings expose the GBP 250 destination'
);
...
select ok(
  has_function_privilege('authenticated', 'public.configure_monzo_easy_bank_transfer_tier_urls(uuid,text,text,text,text,boolean)', 'execute')
  and has_function_privilege('authenticated', 'public.get_monzo_easy_bank_transfer_tier_urls(uuid)', 'execute')
  and not has_function_privilege('authenticated', 'public.resolve_monzo_deposit_redirect(uuid)', 'execute')
  and has_function_privilege('service_role', 'public.resolve_monzo_deposit_redirect(uuid)', 'execute'),
  'RPC grants keep configuration in CRM and redirect resolution backend-only'
);
