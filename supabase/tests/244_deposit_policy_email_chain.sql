...
-- ---------------------------------------------------------------------------
-- 1. Policy copy and provenance schema
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. Real finance RPC path creates an approved deposit request email
-- ---------------------------------------------------------------------------
...
-- Replay the ordinary request. The finance RPC may replay the payment request,
-- but email/outbox dedupe must stay exactly once.
select pg_temp.as_owner();
set local role authenticated;
select lives_ok(
  $$select public.request_session_deposit(
...
-- ---------------------------------------------------------------------------
-- 3. Ordinary payment settlement creates one linked confirmation
-- ---------------------------------------------------------------------------
...
-- Replaying the exact manual transaction must not create another confirmation.
select pg_temp.as_owner();
set local role authenticated;
select lives_ok(
  $$select public.record_manual_payment(
...
-- ---------------------------------------------------------------------------
-- 4. The system-approval exception is narrow
-- ---------------------------------------------------------------------------
...
select throws_ok(
  $$update public.email_messages
      set automation_job_id = (
        select id from public.automation_jobs limit 1
      )
    where payment_request_id = (select id from t_payment)
      and template_key = 'deposit_confirmation'$$,
  '23514', null,
