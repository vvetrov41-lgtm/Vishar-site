# GEO/AEO Project Agent

@.geo-topic-agent-runtime/AGENTS.md

The canonical package root is `.geo-topic-agent-runtime/`. Resolve the execution contract, skills, references, license, and attribution contract under that root.

Use `python .geo-topic-agent-runtime/geo_agent_cli.py <command>`. First setup uses `--runtime claude`.

Guide the user through the canonical long or fast GEO/AEO path, exact approvals, evidence gates, and append-only monitoring.
